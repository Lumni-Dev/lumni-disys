...
const variants: Record<Variant, string> = {
  ...
  outline:
    "border border-border bg-surface-2/40 text-muted shadow-sm hover:border-white/25 hover:bg-surface-2 hover:text-foreground active:scale-[0.98]",
...
export function IconButton({
  ...
  return (
    <button
      {...props}
      className="relative flex h-8 w-8 items-center justify-center rounded-lg border border-border bg-surface text-muted shadow-sm transition-all duration-200 hover:border-white/25 hover:text-foreground active:scale-[0.98]"
    >
      {children}
    </button>
  );
