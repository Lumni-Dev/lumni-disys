import type { Landing } from "@/i18n/types";

const nl: Landing = {
  languageLabel: "Taal",
  nav: {
    modules: "Modules",
    features: "Functies",
    access: "Toegang",
    pricing: "Prijzen",
    signIn: "Inloggen",
  },
  pricing: {
    title: "Plannen en prijzen",
    subtitle: "Begin gratis en upgrade wanneer nodig.",
    cta: "Beginnen",
  },
  hero: {
    titleLead: "Het praktische, snelle",
    titleAccent: "HR-ATS",
    subtitle:
      "Bedrijven, vacatures, kandidaten en processen op een platform, met cv-analyse door AI. Werf beter, van het eerste contact tot de aanwerving.",
    ctaGuest: "Ga naar het systeem",
    ctaAuthed: "Naar het systeem",
    ctaSecondary: "Bekijk modules",
  },
  kpis: {
    companies: "Bedrijven",
    openJobs: "Openstaande vacatures",
    candidates: "Kandidaten",
    inProcess: "In behandeling",
  },
  stats: ["Geïntegreerde modules", "Talen", "Te onthouden wachtwoorden", "Beschikbaar"],
  modulesSection: {
    title: "Alles wat uw HR nodig heeft",
    subtitle:
      "Vijf geïntegreerde modules die met elkaar communiceren en de hele werving in één stroom houden.",
  },
  modules: [
    {
      title: "Bedrijven",
      desc: "Registreer klanten en vestigingen met volledige gegevens en realtime status.",
    },
    {
      title: "Vacatures",
      desc: "Publiceer met volledige omschrijving en deel de publieke link van elke vacature om sollicitaties te ontvangen.",
    },
    {
      title: "Kandidaten",
      desc: "Talentenpool met bijgevoegd cv en door AI berekende match met de vacature, van 0 tot 100.",
    },
    {
      title: "Processen",
      desc: "Visuele drag-and-drop pijplijn; nieuwe sollicitaties komen vanzelf in de screening.",
    },
    {
      title: "Team",
      desc: "Nodig uit per e-mail met acceptatie, stel rechten per pagina in en wissel tussen workspaces.",
    },
  ],
  features: [
    {
      title: "AI-match",
      desc: "Elk binnengekomen cv wordt beoordeeld tegen de vacature, van 0 tot 100.",
    },
    {
      title: "Veilig inloggen",
      desc: "Toegang met alleen Google of LinkedIn, geen wachtwoorden om te beheren.",
    },
    {
      title: "Publieke vacaturepagina's",
      desc: "Deel de link van je workspace of een enkele vacature en ontvang sollicitaties met cv.",
    },
  ],
  cta: {
    title: "Begin slimmer te werven",
    subtitle: "Log in met uw account en krijg direct toegang tot het hele platform.",
  },
  footer: {
    description:
      "Lumni bouwt systemen, automatiseert processen en integreert engineers in uw team. Van de eerste diagnose tot code die draait in productie.",
    servicesHeading: "Diensten",
    services: [
      "Ontwikkeling van systemen en applicaties",
      "Procesautomatisering",
      "Technologieadvies",
      "Cyberbeveiliging",
      "Ontwikkelingsondersteuning",
    ],
    contactHeading: "Contact",
    privacy: "Privacy",
    terms: "Voorwaarden",
    backToTop: "Terug naar boven",
    rights: "Alle rechten voorbehouden.",
  },
};

export default nl;
